...
1. Execute full Maven build to create the `target/MSurakkaAssignment.war` file:
    ```bash
    $ mvn clean install
    ```
...
2. Download and install Liberty, then use it to run the built application from step 1:
    ...
    Once the server is running, the application will be available under [http://localhost:9080/MSurakkaAssignment](http://localhost:9080/MSurakkaAssignment).

Use the following command to run the built application in Bluemix:
    ```bash
    $ cf push <appname> -p target/MSurakkaAssignment.war
    ```
...
4. Run your application locally on Liberty:
  - Right click on the `MSurakkaAssignment` sample and select `Run As` -> `Run on Server` option
  - Find and select the localhost Liberty server and press `Finish`
  - In a few seconds, your application should be running at http://localhost:9080/MSurakkaAssignment/
